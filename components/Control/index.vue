...
const test2 = (sortType) => {
  if (sortType === 'defolt') {
    ...
    return serchNotes.value.sort((a, b) => {>
      if (a.date > b.date) {
        return -1;
      }
      if (a.date < b.date) {
...
const serchNotes = computed(() => {
  ...
  return notes.value.filter(note => note.title.toLowerCase().includes(searchValue.value.toLowerCase()))
  }
  else {
    return notes.value
  }
...
<template>
  <div>
    <div class="flex-none">
      <div 
        class="flex justify-between align-middle mt-3 mb-3 pr-4 pl-4"
      >
        <UI-input
          @chenge="serchNotes"
          type="text" 
